        ...
        for tpg in for_each_iscsi_tpg(tiqn):
            print(
                "{}o- {} (struct se_portal_group {})".format(
                    indent * 2,
                    get_tpg_name(tpg.tpg_se_tpg),
                    hex(tpg.tpg_se_tpg.address_of_()),
                )
            )
            print(f"{indent * 3}o- acls")
            for acl in list_for_each_entry(
                "struct se_node_acl",
                tpg.tpg_se_tpg.acl_node_list.address_of_(),
                "acl_list",
            ):
                print(
            ...
            for lun in hlist_for_each_entry(
                "struct se_lun",
                tpg.tpg_se_tpg.tpg_lun_hlist.address_of_(),
                "link",
            ):
...
def print_vhost_info(prog) -> None:
    ...
    for tpg in for_each_vhost_tpg(prog):
        ...
        for acl in list_for_each_entry(
            "struct se_node_acl",
            tpg.se_tpg.acl_node_list.address_of_(),
            "acl_list",
        ):
            if not acl:
                break
