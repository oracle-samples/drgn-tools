...
def show_multipath(prog: Program, target: Object) -> None:
    ...
    if has_member(multipath, "queue_mode"):
...
def dump_hash_cell(prog: Program, hc: Object) -> None:
    ...
    if target_type == "multipath":
        print(
            "%s Device [%d] %s" % (hyphen * 15, device_count[0], hyphen * 15)
        )
...
def dump_uek4_5_6_multipaths(prog: Program) -> None:
    """
