...
def test_list_lru(
    ...
    *,
    ...
    This is an exhastive search of every entry.
    """
        ...
        for nid, mcgid, dentry in list_lru_for_each_entry(
            ...
            if (memcg == -1) or (mcgid != memcg) or (n != nid):
                if verbose is not None:
                    print(
                        f"memcg/nodeid differ dentry {hex(dentry)} {hex(mcgid)} {hex(memcg)} {hex(n)} {hex(nid)}"
                    )
                errors = errors + 1
            else:
                if verify is not None:
                    ...
                    if found == 0:
                        if verbose is not None:
                            print(
                                f"lru for dentry {hex(dentry)} NOT found at memcg idx {memcg.value_()} nodeid {n.value_()}"
                            )
                        errors = errors + 1
                    else:
                        if verbose is not None:
        ...
        if mnt_fstype == "xfs":
            ...
            for nid, mcgid, bp in list_lru_for_each_entry(
                ...
                if (memcg == -1) or (mcgid != memcg) or (n != nid):
                    if verbose is not None:
                        print(
                            f"memcg/nodeid differ xfsbuf {hex(bp)} {hex(mcgid)} {hex(memcg)} {hex(n)} {hex(nid)}"
                        )
                    errors = errors + 1
                else:
                    if verify is not None:
                        ...
                        if found == 0:
                            if verbose is not None:
                                print(
                                    f"lru for xfs_buf {hex(bp)} NOT found at memcg idx {memcg} nodeid {n.value_()}"
                                )
                            errors = errors + 1
                        else:
                            if verbose is not None:
