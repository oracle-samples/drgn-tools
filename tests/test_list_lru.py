...
def test_list_lru(
    ...
    *,
    ...
    This is an exhastive search of every entry. The optional arguement,
    maxitems limits the number of items searched per filesystem.
    """
    for mnt in for_each_mount(
        ...
        for nid, mcgid, dentry in list_lru_for_each_entry(
            ...
            d_cnt = d_cnt + 1
            if (maxitems is not None) and (d_cnt >= maxitems):
                # limit the items searched if provided a maxitems
                break
