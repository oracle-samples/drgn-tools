...
class TestNumastat(DrgnToolsTestCase):
    def test_numastat(self):
        numastat.show_all_nodes_meminfo(self.prog)

    @skip_unless_live
    def test_numastat_all_nodes_meminfo(self):
        page_shift = self.prog.constant("PAGE_SHIFT").value_()
        numastat_per_node_mm_stats = []
        active_nodes = meminfo.get_active_numa_nodes(self.prog)
        num_active_nodes = len(active_nodes)

        for node in active_nodes:
            node_mm_stats = numastat.get_per_node_meminfo(self.prog, node)
