...
class TestMeminfo(DrgnToolsTestCase):
    def test_meminfo_smoke(self):
        meminfo.show_all_meminfo(self.prog)

    @skip_unless_live
    def test_meminfo(self):
        page_shift = self.prog.constant("PAGE_SHIFT").value_()

        # Get mm statistics from the live vmcore.
        corelens_mm_stats = meminfo.get_all_meminfo(self.prog)
        ...
        if self.prog.platform.arch == drgn.Architecture.X86_64:
        ...
        elif self.prog.platform.arch == drgn.Architecture.AARCH64:
            test_exact_match_mm_stats = [
                "MemTotal",
                "SwapTotal",
                "CommitLimit",
